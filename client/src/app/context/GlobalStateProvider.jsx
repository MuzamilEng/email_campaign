...
export const GlobalStateProvider = ({ children }) => {
  ...
  const filterByMonth = (invoice) => {
    ...
    return invoice?.month?.substring(0, 3).toLowerCase() === searchMonth.substring(0, 3).toLowerCase();
  };
  
  const handleMonthChange = (event) => {
    const selectedMonth = event.target.value;
    setSearchMonth(selectedMonth);
    const getInvoiceByMonth = monthlyInvoice?.filter((invoice) => 
      invoice?.month?.substring(0, 3).toLowerCase() === selectedMonth.substring(0, 3).toLowerCase()
    );
  ...
  return (
    <GlobalStateContext.Provider
      value={{
        ...
        globalAdminData,
        setGlobalAdminData,
        userReport,setUserReport, monthlyInvoice, setMonthlyInvoice,
        currentUserInvoiveData, setCurrentUserInvoiveData, months, filterByMonth, handleMonthChange
      }}
    >
      {children}
    </GlobalStateContext.Provider>
