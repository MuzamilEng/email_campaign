...
const Form = () => {
  ...
  const [campaignDetails, setCampaignDetails] = useState({ noOfPoints: "",  message:"", startDate: dayjs(null), endDate: dayjs(null), file: null });
    ...
    <main className="flex w-full justify-center p-[1vw] items-center">
      ...
      <Box className=''>
			...
			<Dialog onClose={handleClose} aria-labelledby='customized-dialog-title' open={open}>
				<DialogTitle id='customized-dialog-title'>Initialize Campaign</DialogTitle>
       ...
       <span className="text-[0.8vw] font-medium mt-[0.7vw] text-gray-600">Upload up to 1 file at once. Upgrade for more</span>
        ...
        <span onClick={() => setShowPointFields(!showPointFields)} className="text-[0.8vw] cursor-pointer hover:underline font-medium mt-[0.7vw] text-gray-600">{!showPointFields ? "or use no of points." : "upload your file"}</span>
       </section> 
						{showPointFields && <TextField
							autoFocus
							margin='dense'
							id='noOfPoints'
							label='No of points'
							...
							onChange={(event) => setCampaignDetails({...campaignDetails, noOfPoints: event.target.value})}
            ...
            setStartDate={(date) => setCampaignDetails({ ...campaignDetails, startDate: date })}
            setEndDate={(date) => setCampaignDetails({ ...campaignDetails, endDate: date })}
          />
          </div>
						<TextField
							margin='dense'
              ...
              required
							onChange={(event) =>
                setCampaignDetails({...campaignDetails, message: event.target.value})
							}
						/>
					</form>
				</DialogContent>
				<DialogActions>
					...
					<Button variant='contained' type='submit' form='createBoardForm' onClick={handleSubmit(onSubmit)}>
            submit
					</Button>
				</DialogActions>
			</Dialog>
